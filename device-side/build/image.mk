...
ifneq ($(CONFIG_SYSTEM_INIT_BUSYBOX)$(CONFIG_BOOT_TIME_OPTIMIZATION),yy)
  SQUASHFSOPT += -p '/dev d 755 0 0' -p '/dev/console c 600 0 0 5 1'
else
  SQUASHFSOPT += -pf $(BUILD_DIR)/device_table_boot_optimization.txt
endif
SQUASHFSCOMP := gzip
...
ifneq ($(CONFIG_SUNXI_SMALL_STORAGE_OTA),)
	...
	$(STAGING_DIR_HOST)/bin/mksquashfs4 ./temp_usr $(KDIR)/usr.squashfs -noappend -root-owned -comp $(SQUASHFS_COMP) $(SQUASHFSOPT) -processors $(if $(CONFIG_PKG_BUILD_JOBS),$(CONFIG_PKG_BUILD_JOBS),1) $(if $(SOURCE_DATE_EPOCH),-fixed-time $(SOURCE_DATE_EPOCH))
endif
	$(STAGING_DIR_HOST)/bin/mksquashfs4 $(TARGET_DIR) $(KDIR)/root.squashfs -noappend -root-owned -comp $(SQUASHFS_COMP) $(SQUASHFSOPT) -processors $(if $(CONFIG_PKG_BUILD_JOBS),$(CONFIG_PKG_BUILD_JOBS),1) $(if $(SOURCE_DATE_EPOCH),-fixed-time $(SOURCE_DATE_EPOCH))
ifneq ($(CONFIG_SUNXI_SMALL_STORAGE_OTA),)
