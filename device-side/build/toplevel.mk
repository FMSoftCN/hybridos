...
prepare-tmpinfo: FORCE
	...
	$(_SINGLE)$(NO_TRACE_MAKE) -j1 -r -s -f build/scan.mk SCAN_TARGET="targetinfo" SCAN_DIR="target/linux" SCAN_NAME="target" SCAN_DEPS="*.mk $(TOPDIR)/build/kernel*.mk $(TOPDIR)/build/target.mk" SCAN_DEPTH=2 SCAN_EXTRA="" SCAN_MAKEOPTS="TARGET_BUILD=1"
...
.config: ./scripts/config/conf $(if $(CONFIG_HAVE_DOT_CONFIG),,prepare-tmpinfo)
	@+if [ \! -e .config ] || ! grep CONFIG_HAVE_DOT_CONFIG .config >/dev/null; then \
		if [ "x$(TARGET_BUILD_VARIANT)" = "xtina" ]; then \
			[ -e target/linux/$(TARGET_BOARD)/defconfig ] && \
			cp target/linux/$(TARGET_BOARD)/defconfig .config; \
		elif [ "x$(TARGET_BUILD_VARIANT)" = "xdragonboard" ]; then \
			[ -e target/linux/$(TARGET_BOARD)/defconfig-db ] && \
			cp target/linux/$(TARGET_BOARD)/defconfig-db .config; \
		fi \
...
menuconfig: scripts/config/mconf prepare-tmpinfo FORCE
	if [ -z $(TARGET_BOARD) ]; then \
		...
		if cat .config|grep CONFIG_TARGET_BOARD=\"$(TARGET_BOARD)\" >> /dev/null; then \
			[ "x$(TARGET_BUILD_VARIANT)" = "xtina" ] && \
			cp .config target/linux/$(TARGET_BOARD)/defconfig; \
			[ "x$(TARGET_BUILD_VARIANT)" = "xdragonboard" ] && \
			cp .config target/linux/$(TARGET_BOARD)/defconfig-db; \
			echo ; \
		else \
			rm .config; \
		fi \
...
ota_menuconfig: scripts/config/mconf prepare-tmpinfo FORCE
	if [ -z $(TARGET_BOARD) ]; then \
		...
		[ "x$(TARGET_BUILD_VARIANT)" = "xtina" ] && \
		[ -e target/linux/$(TARGET_BOARD)/defconfig_ota ] && \
		cp target/linux/$(TARGET_BOARD)/defconfig_ota .config; \
		$< Config.in; \
		if cat .config|grep CONFIG_TARGET_BOARD=\"$(TARGET_BOARD)\" >> /dev/null; then \
			[ "x$(TARGET_BUILD_VARIANT)" = "xtina" ] && \
			cp .config target/linux/$(TARGET_BOARD)/defconfig_ota; \
			echo ; \
		else \
			rm .config; \
		fi \
