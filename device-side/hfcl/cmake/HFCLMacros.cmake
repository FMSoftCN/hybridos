...
macro(HFCL_COMPUTE_SOURCES _framework)
...
macro(HFCL_INCLUDE_CONFIG_FILES_IF_EXISTS)
...
macro(HFCL_ADD_SOURCE_DEPENDENCIES _source _deps)
    set(_tmp)
...
macro(HFCL_ADD_PRECOMPILED_HEADER _header _cpp _source)
    if (MSVC)
        ...
        foreach (_src ${_sources})
            HFCL_ADD_SOURCE_DEPENDENCIES(${_src} ${PrecompiledBinary})
        endforeach ()
...
macro(HFCL_FRAMEWORK_DECLARE _target)
...
macro(HFCL_EXECUTABLE_DECLARE _target)
    add_executable(${_target} "${CMAKE_BINARY_DIR}/cmakeconfig.h")
endmacro()

# Private macro for setting the properties of a target.
# Rather than just having _target like HFCL_FRAMEWORK and HFCL_EXECUTABLE the parameters are
# split into _target_logical_name, which is used for variable expansion, and _target_cmake_name.
# This is done to support HFCL_WRAP_EXECUTABLE which uses the _target_logical_name variables
# but requires a different _target_cmake_name.
macro(_HFCL_TARGET _target_logical_name _target_cmake_name)
...
macro(HFCL_FRAMEWORK _target)
    _HFCL_TARGET(${_target} ${_target})
...
macro(HFCL_WRAP_EXECUTABLE _target)
    ...
    _HFCL_TARGET(${_target} ${_wrapped_target_name})
...
macro(HFCL_CREATE_FORWARDING_HEADER _target_directory _file)
    ...
    # Hfcl2 generate-forwarding-headers script.
...
macro(HFCL_CREATE_FORWARDING_HEADERS _framework)
    ...
    if (NOT WIN32)
        ...
        foreach (_currentArg ${ARGN})
            ...
            elseif (_processing_directories)
                ...
                foreach (_file ${_files})
                    HFCL_CREATE_FORWARDING_HEADER(${_target_directory} ${_file})
                endforeach ()
            elseif (_processing_files)
                HFCL_CREATE_FORWARDING_HEADER(${_target_directory} ${_currentArg})
            endif ()
        endforeach ()
    endif ()
endmacro()

function(HFCL_MAKE_FORWARDING_HEADERS framework)
...
macro(HFCL_DEBUG_DUMP_COMMANDS)
    set(CMAKE_VERBOSE_MAKEFILE ON)
endmacro()

macro(HFCL_DEBUG_DUMP_VARIABLES)
...
macro(HFCL_ADD_TARGET_PROPERTIES _target _property _flags)
...
macro(HFCL_POPULATE_LIBRARY_VERSION library_name)
...
macro(HFCL_CREATE_SYMLINK target src dest)
