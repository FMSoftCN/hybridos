 ...
 * \date 2020/09/16
 *
 * \brief This file implements user compositor.
 *
 \verbatim
...
struct _CompositorCtxt {
    ...
    int  iAppNumber;    // the number of application on NOMAL level
...
static void refresh(CompositorCtxt* ctxt)
{
    ...
    if(idx_topmost > 0)
    {
        ...
        if(znodeheader)
        {
printf("=======================2222222 %d, %d\n", znodeheader->rc.right, znodeheader->rc.bottom);
            BitBlt(znodeheader->mem_dc, 0, 0, znodeheader->rc.right, znodeheader->rc.bottom, HDC_SCREEN_SYS, 100, 50, 0);
