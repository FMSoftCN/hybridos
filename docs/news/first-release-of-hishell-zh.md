...
（插入视频）  
在 hiWebKit 的支持下，在 hiShell 环境中运行 hiApp
...
定制窗口合成器，以九宫格方式展示系统中的应用窗口，并提供了常见于现代智能设备的窗口切换功能和动画特效
...
1. hiBusServer 本地数据总线服务器。通过该服务器，我们可以在应用中获得系统状态的变化信息（如电量、WiFi 信号强度等），还可以通过发送任务请求来完成某些系统功能，如连接到指定的 WiFi 热点。开发者还可以编写自己的模块提供总线服务。另外，该服务器将同时在 Unix Domain Socket 和本地的 Web Socket 上提供服务，以方便在 hiApp 中使用这些系统和应用提供的服务。
1. 移植 OpenHarmoney 2.0 中的 JavaScript 应用框架，并将我们在 hiWebKit 中的突破技术整合到该框架中，以便为某些应用场景提供轻量级的类 Web 应用支持。
