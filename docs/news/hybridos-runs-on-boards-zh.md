...
1. hiWebKit：用于支撑 HTML5 及 HybridOS 扩展特性的 WebKit 扩展版本：
   - 其中用于展示手表表盘、秒表的页面，采用 hiWebKit 的 `hiview` 扩展标签实现。仔细观察表盘指针的运动，在两款开发板上运行的效果都非常棒。
   - 其中用于展示仪表盘的页面，使用了 hiWebKit 的 `himeter` 和 `hihand` 扩展标签实现。仔细观察仪表盘中的指针及条带变化，在两款开发板上的运行效果都非常棒。
   - 目前应用的启动速度 R818 比 PX30 要快一些，但理论上应该是一样的，尚需要进一步优化。

如果读者手头有这两块开发板，可以阅读如下内容来下载 HybridOS 设备侧 R2011 的已编译映像文件，放到开发板上运行。
...
6) 当 PX30 开发板重新启动后，既可以看到 HybridOS 界面，并可以通过触摸屏进行操作。

## 吐槽

## 后续计划

飞漫的研发团队正在开发合璧操作系统设备侧的第一个正式版本，在第一个正式版中，将新增如下特性：
