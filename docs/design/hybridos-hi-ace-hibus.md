...
constructor(app, runner) ：hiBus_handler类的构造函数，在该函数中，创建hiBusConnection对象。
...
​        无。如果hiBus连接成功，则hiBusConnection为hiBus对象，否则hiBusConnection为undefined。
 ...
 fireEvent(name, data)：  产生一个事件，并发到hiBus总线上。
...
subscribeEvent(app, runner, event, func)：订阅一个事件，并发到hiBus总线
...
revokeEvent(app, runner, event)：取消订阅一个事件，，并发到hiBus总线
...
registerProcedure(method, func) { ......}：向hiBus注册过程
...
revokeProcedure(method)：向hiBus发送撤销过程
...
callProcedure(app, runner, method, param, timeout)：向hiBus发送异步远程过程调用请求
...
callProcedureAndWait(app, runner, method, param, timeout) ：向hiBus发送同步远程过程调用请求
...
hiBus.subscribeEvent("battery-daemon", "battery-runner", "battery-quantity", display() { ...... });

// 读取hiBus总线，解析数据，如果“battery-quantity”消息存在，则执行display() {}
hiBus.hibusHandler();
...
bool hiBusConnection(char * app_ame, char * runner_name)：hiBusConnection类的构造函数，创建到hiBus总线的连接。
...
int hiBusReadRaw(char * buffer, int length)：对hiBus总线的读操作
...
int hiBusWriteRaw(char * buffer, int length)：对hiBus总线的写操作
