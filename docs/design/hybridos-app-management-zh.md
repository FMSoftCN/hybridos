...
根据[合璧操作系统设备端数据总线概要设计](hybridos-data-bus-zh.md)文档的描述，在合璧操作系统设备端，一个应用可能存在多个并行运行的实体（进程或者线程），每个独立运行的实体称为`行者`（Runner）。

一个行者可能属于如下类型之一：

* 守护进程（daemon）：指应用启动时会自动启动并在后台执行的行者。
* 交互进程（ui-runner）：指负责和最终用户交互的进程，一般情况下就是一个图形用户界面（GUI）进程。在没有屏幕的设备上，可能是语音交互进程。
* 临时进程：指其他行者临时启动的进程，在完成指定的任务后退出。
* 线程：依附于以上进程的一个线程。
...
和其他系统不同的是，在 HybridOS 中，我们引入了应用组的概念：
