...
|                 Unix Domain Socket Connection Manager              |
 --------------------------------------------------------------------
   ...
   |--------------------      |--------------------
   | Event handlers     |     |      Module 1      |
   | Result handlers    |     |      Module 2      |
   |                    |     |        ...         |
    --------------------       --------------------
```

如上图所示，App 通过 Web Socket 或者 Unix Domain Socket 连接到 hiBus 服务器，每个应用可包含若干模块，可用于过程处理，产生事件，或者处理事件，发起过程调用。

其中，hiBus 命令行（command line）作为 hibus 应用的外挂模块实现，可用于系统管理员查看或者开发者调试使用。

hiBus 服务器主要包含如下软件模块：

- Web Socket/Unix Domain Socket 连接管理器：该模块用于监听套接字端口，接受连接请求，检测连接是否已断开等。
- 连接/端点管理器（Connection/Endpoint Manager）：该模块记录每个连接对应的端点名（含主机名、应用名以及模块名）。
...
- 辅助模块（Helpers）：辅助模块中主要包括 JSON 的处理模块以及安全性处理模块。后者主要用来处理应用身份验证、主机及应用过滤等。
